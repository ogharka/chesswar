...
const NFT_TIERS = [
  { tier: 1, name: 'Soldier',   boost: '2×', price: '0.005 ETH', color: '#0052FF',
  ...
  { tier: 2, name: 'Knight',    boost: '3×', price: '0.01 ETH',  color: '#05B169',
  ...
  { tier: 3, name: 'Commander', boost: '4×', price: '0.025 ETH', color: '#8B5CF6',
  ...
  { tier: 4, name: 'Warlord',   boost: '5×', price: '0.05 ETH',  color: '#C9A84C',
...
export default function ProfilePage() {
  ...
  const handleMint = async (tier) => {
    ...
    if (!nftAddress || !window.ethereum) { toast.error('Wallet not connected'); return; }
    setMinting(tier);
    try {
      const provider  = new ethers.BrowserProvider(window.ethereum);
